...
- **مصدر التوكنز الوحيد: `lib/theme.ts`** — يصدّر `C` (ألوان)، `R` (حواف)، `T` (أدوات Tailwind)، `STAGE_THEMES`. القيم الموسومة `[uula]` مؤكَّدة من نسخة uula.com/kw محفوظة محليًا في `.playwright-capture/` (مستثنى من git) وتقريرها `tokens-report.txt`؛ الموسومة `[tafawwuq]` قراراتنا (أسطح الخ الداكنة غير مؤكَّدة لأن ملف CSS الخاص بـ uula خارجي ولم يُحفظ).
- **الخط:** `app/layout.tsx` يحمّل Tajawal عبر `next/font/google` تحت الاسم `--font-body`، و`@theme` في `globals.css` يعرّف `--font-sans: var(--font-body), …`. **لا تُعد استخدام `--font-sans` كاسمًا لمتغير next/font** — التصادم كان يسقط الواجهة على خط النظام (وكان الاسم يشير لـ "Alexandria" غير الموجود أصلًا).
- الهوية البصرية: داكنة بأسلوب UULA — خلفية `#0f1217`، أسطح `#161c29`/`#1a2130`، حدود `#2b3547`، أزرق `#2072e0` (hover `#1b63c4`، نص فاتح `#4a9bf5`)، **نص ثانوي `#9297a6` ونص هامشي `#5f6370` (كلاهما من uula — لا تُدخل درجات رمادية جديدة)**، ذهبي `#f5b329`، خط Tajawal، RTL. لا سيان في الهوية: التدرجات النصية والهالة تستخدم الأزرق `#2072e0`. كل الصفحات (طالب/ولي أمر/أدمن) على `AppShell dark`.
