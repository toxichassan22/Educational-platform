    ...
    <AppShell role="student" dark>
      <div className="space-y-8 animate-fade-up">
        <div className="text-center pt-4">
          ...
          <p className="text-[#99a8bd]">كل الباقات تشمل الفيديوهات والمذكرات والاختبارات</p>
        </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-xs text-[#99a8bd] mt-1">
              ...
              <div className="text-left">
                <div className="text-xs text-[#99a8bd]">التجديد التلقائي</div>
            ...
            return (
              <div key={p.id}
                ...
                <div className="text-center my-5 flex items-baseline justify-center gap-1.5" dir="ltr">
                  ...
                  <span className="text-sm font-bold text-[#99a8bd]">د.ك</span>
                </div>
                <div className="text-center text-sm font-bold text-[#99a8bd] mb-5 -mt-3">{p.period}</div>
                ...
                <button disabled={isCurrent}
                  ...
                  className={`w-full h-[52px] rounded-full font-black text-sm transition-all active:scale-[.98] disabled:opacity-60 ${
                    isCurrent ? "bg-[#1a2130] border border-[#2b3547] text-[#99a8bd]"
        ...
        <p className="text-center text-sm font-bold text-[#99a8bd]">دفع آمن عبر KNET · Apple Pay · Visa</p>

        {/* سجل المدفوعات */}
        <DCard className="p-6 max-w-3xl mx-auto w-full">
          <h2 className="font-black mb-4">سجل المدفوعات</h2>
          {myPayments.length === 0 && <p className="text-sm text-[#99a8bd]">لا توجد مدفوعات سابقة</p>}
            ...
            {myPayments.map((p) => (
              <div key={p.id} className="flex items-center gap-3 p-3 rounded-xl bg-[#1a2130]">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[11px] text-[#99a8bd]">{p.date} · {p.method}</div>
                </div>
          ...
          <div className="space-y-4">
            <div className="bg-[#1a2130] rounded-xl p-4 flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-[#99a8bd]">{sel.period}</div>
              </div>
            ...
            {sel.scope === "subject" && (
              <div>
                <label className="text-xs font-bold text-[#99a8bd] block mb-1.5">اختر المادة اللي تفتحها</label>
                <div className="grid grid-cols-2 gap-2">
                  {mySubjects.map((s) => (
                    <button key={s.id} type="button" onClick={() => setSubjPick(s.id)}
                      className={`flex items-center gap-2 p-2.5 rounded-xl border-2 text-sm font-bold transition-all ${subjPick === s.id ? "border-[#2072e0] bg-[#1a3454] text-white" : "border-[#2b3547] text-[#99a8bd] hover:border-[#2072e0]/50"}`}>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input value={code} onChange={(e) => setCode(e.target.value)} dir="ltr" placeholder="XXXX"
                  className="flex-1 bg-[#1a2130] border border-[#2b3547] rounded-xl px-4 py-2.5 text-sm text-white outline-none focus:border-[#2072e0] text-left placeholder:text-[#99a8bd]/50" />
              ...
              <div className="grid grid-cols-3 gap-2">
                {(["KNET", "Visa", "Mastercard"] as const).map((m) => (
                  <button key={m} onClick={() => setMethod(m)}
                    className={`py-3 rounded-xl border-2 font-black text-sm transition-all ${method === m ? "border-[#2072e0] bg-[#1a3454] text-[#4a9bf5]" : "border-[#2b3547] text-[#99a8bd]"}`}>
                    {m}
                  </button>
                ))}
              </div>
            ...
            <div className="border-t border-[#2b3547] pt-3 space-y-1.5 text-sm">
              <div className="flex justify-between text-[#99a8bd]"><span>السعر</span><span>{sel.priceKwd} د.ك</span></div>
        ...
        {sel && step === "pay" && (
          <div className="space-y-4">
            <div className="rounded-2xl p-4 text-white flex items-center justify-between bg-[#1a2c4d] border border-[#2072e0]/40">
              <div>
                <div className="text-[10px] text-[#99a8bd]">إجمالي المبلغ</div>
            ...
            {method === "KNET" ? (
              <div className="space-y-3">
                ...
                <input dir="ltr" placeholder="رقم البطاقة" className="w-full bg-[#1a2130] border border-[#2b3547] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-[#2072e0] text-left placeholder:text-[#99a8bd]/50" />
                <div className="grid grid-cols-2 gap-3">
                  <input dir="ltr" placeholder="MM/YY" className="bg-[#1a2130] border border-[#2b3547] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-[#2072e0] text-left placeholder:text-[#99a8bd]/50" />
                  <input dir="ltr" placeholder="PIN" type="password" className="bg-[#1a2130] border border-[#2b3547] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-[#2072e0] text-left placeholder:text-[#99a8bd]/50" />
                </div>
              </div>
            ) : (
              <div className="space-y-3">
                <input dir="ltr" placeholder="رقم البطاقة  ••••  ••••  ••••" className="w-full bg-[#1a2130] border border-[#2b3547] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-[#2072e0] text-left placeholder:text-[#99a8bd]/50" />
                <input placeholder="الاسم على البطاقة" className="w-full bg-[#1a2130] border border-[#2b3547] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-[#2072e0] placeholder:text-[#99a8bd]/50" />
                <div className="grid grid-cols-2 gap-3">
                  <input dir="ltr" placeholder="MM/YY" className="bg-[#1a2130] border border-[#2b3547] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-[#2072e0] text-left placeholder:text-[#99a8bd]/50" />
                  <input dir="ltr" placeholder="CVV" type="password" className="bg-[#1a2130] border border-[#2b3547] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-[#2072e0] text-left placeholder:text-[#99a8bd]/50" />
                </div>
              </div>
            )}
            ...
            <button onClick={() => setStep("choose")} className="w-full text-xs text-[#99a8bd] font-bold">← رجوع</button>
          </div>
        )}
      ...
      <Modal dark open={step === "done"} onClose={() => { setSel(null); setStep("choose"); }} title="">
        <div className="text-center py-4">
          ...
          <p className="text-sm text-[#99a8bd] mb-1">اشتراكك في «{sel?.name}» مفعّل الآن</p>
          <p className="text-xs text-[#99a8bd]/70 mb-6">أُرسل إشعار تأكيد إلى بريدك الإلكتروني</p>
