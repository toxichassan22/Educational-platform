  ...
  return (
    <div className="grid lg:grid-cols-5 gap-5">
          ...
          <div className="flex gap-1.5 mb-3">
            {db.stages.map((s) => (
              <button key={s.id} onClick={() => { setStageId(s.id); const g = db.grades.find(x => x.stageId === s.id)!; setGradeId(g.id); setSubjectId(null); }}
                className={`flex-1 py-1.5 rounded-lg text-xs font-bold ${stageId === s.id ? "bg-[#2072e0] text-white" : "bg-[#1a2130] text-[#99a8bd]"}`}>
          ...
          <div className="flex gap-1.5 mb-4 overflow-x-auto pb-1">
            {grades.map((g) => (
              <button key={g.id} onClick={() => { setGradeId(g.id); setSubjectId(null); }}
                className={`px-3 py-1.5 rounded-lg text-[11px] font-bold whitespace-nowrap ${gradeId === g.id ? "bg-[#2072e0] text-white" : "bg-[#1a2130] text-[#99a8bd]"}`}>
                {g.name.replace("الصف ", "")}
              </button>
            ))}
          </div>
              ...
              return (
                <div key={s.id} onClick={() => setSubjectId(s.id)}
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[10px] text-[#99a8bd]">{uCount} وحدات · {lCount} درسًا · {s.teacher}</div>
                  </div>
                  <button onClick={(e) => { e.stopPropagation(); deleteSubject(s.id); if (subjectId === s.id) setSubjectId(null); }}
                    className="p-1.5 rounded-lg text-[#5b6478] hover:bg-red-500/15 hover:text-red-400"><Icon name="trash" size={15} /></button>
                </div>
              );
            })}
            {subjects.length === 0 && <p className="text-xs text-[#99a8bd] text-center py-4">لا مواد — أضف أول مادة</p>}
          </div>
        </Card>
      </div>
        ...
        <Card className="p-4 border border-[#2b3547] min-h-[300px]">
          {!subject ? (
            <div className="h-full flex flex-col items-center justify-center text-[#5b6478] py-16">
              ...
              <p className="text-sm mt-3 text-[#99a8bd]">اختر مادة لعرض وحداتها ودروسها</p>
            </div>
          ) : (
            <>
                  ...
                  <div key={u.id} className="border border-[#2b3547] rounded-xl overflow-hidden">
                    ...
                    {lessonsOfUnit(db, u.id).map((l) => (
                      <div key={l.id} className="flex items-center gap-3 px-4 py-2.5 border-t border-white/[0.07]">
                        <Icon name="video" size={15} className="text-[#5b6478]" />
                        <span className="flex-1 text-sm text-white/90">{l.title}</span>
                        <span className="text-[10px] text-[#99a8bd]">{l.durationMin}د</span>
                        <Badge tone="blue">{questionsOfLesson(db, l.id).length} سؤال</Badge>
                        <button onClick={() => deleteLesson(l.id)} className="p-1 rounded-lg text-[#5b6478] hover:bg-red-500/15 hover:text-red-400"><Icon name="trash" size={14} /></button>
                      </div>
                    ))}
                  </div>
                ))}
                {units.length === 0 && <p className="text-xs text-[#99a8bd] text-center py-8">لا وحدات بعد</p>}
              </div>
            </>
          )}
        </Card>
      ...
      <Modal open={addSubOpen} onClose={() => setAddSubOpen(false)} title="إضافة مادة جديدة">
        <div className="space-y-3">
          <div>
            <label className="text-xs font-bold text-[#99a8bd] block mb-1">اسم المادة</label>
          ...
          <div>
            <label className="text-xs font-bold text-[#99a8bd] block mb-1">المعلم</label>
            <input value={subTeacher} onChange={(e) => setSubTeacher(e.target.value)} placeholder="أ. الاسم"
              className="w-full border border-[#2b3547] rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#2072e0]" />
          </div>
          <div className="text-xs text-[#99a8bd] bg-[#1a2130] rounded-lg p-2">
      ...
      <Modal open={addUnitOpen} onClose={() => setAddUnitOpen(false)} title="إضافة وحدة جديدة">
        <div className="space-y-3">
          <div>
            <label className="text-xs font-bold text-[#99a8bd] block mb-1">اسم الوحدة</label>
      ...
      <Modal open={!!addLessonOpen} onClose={() => setAddLessonOpen(null)} title="إضافة درس جديد">
        <div className="space-y-3">
          <div>
            <label className="text-xs font-bold text-[#99a8bd] block mb-1">عنوان الدرس</label>
            <input value={lessonTitle} onChange={(e) => setLessonTitle(e.target.value)} placeholder="مثال: قانون نيوتن الأول"
              className="w-full border border-[#2b3547] rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#2072e0]" />
          </div>
          <div>
            <label className="text-xs font-bold text-[#99a8bd] block mb-1">المدة (دقائق)</label>
