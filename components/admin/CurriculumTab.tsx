  ...
  return (
    <div className="grid lg:grid-cols-5 gap-5">
      ...
      <div className="lg:col-span-2 space-y-4">
        <Card className="p-4 border border-slate-100">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-extrabold text-primary-900 text-sm">الهيكل الدراسي</h3>
          ...
          <div className="flex gap-1.5 mb-3">
            {db.stages.map((s) => (
              <button key={s.id} onClick={() => { setStageId(s.id); const g = db.grades.find(x => x.stageId === s.id)!; setGradeId(g.id); setSubjectId(null); }}
                className={`flex-1 py-1.5 rounded-lg text-xs font-bold ${stageId === s.id ? "bg-primary-600 text-white" : "bg-slate-100 text-slate-500"}`}>
          ...
          <div className="flex gap-1.5 mb-4 overflow-x-auto pb-1">
            {grades.map((g) => (
              <button key={g.id} onClick={() => { setGradeId(g.id); setSubjectId(null); }}
                className={`px-3 py-1.5 rounded-lg text-[11px] font-bold whitespace-nowrap ${gradeId === g.id ? "bg-primary-800 text-white" : "bg-slate-100 text-slate-500"}`}>
                {g.name.replace("الصف ", "")}
              </button>
            ))}
          </div>
            ...
            {subjects.map((s) => {
              ...
              return (
                <div key={s.id} onClick={() => setSubjectId(s.id)}
                  className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer border-2 transition-all ${subjectId === s.id ? "border-primary-400 bg-primary-50/50" : "border-slate-100 hover:border-primary-200"}`}>
                  <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: `${s.color}15`, color: s.color }}>
                    <Icon name={s.icon} size={17} />
                  </div>
                  <div className="flex-1">
                    <div className="font-bold text-sm text-primary-900">{s.name}</div>
                    <div className="text-[10px] text-slate-400">{uCount} وحدات · {lCount} درسًا · {s.teacher}</div>
                  </div>
                  <button onClick={(e) => { e.stopPropagation(); deleteSubject(s.id); if (subjectId === s.id) setSubjectId(null); }}
                    className="p-1.5 rounded-lg text-slate-300 hover:bg-red-50 hover:text-red-500"><Icon name="trash" size={15} /></button>
                </div>
              );
            })}
            {subjects.length === 0 && <p className="text-xs text-slate-400 text-center py-4">لا مواد — أضف أول مادة</p>}
          </div>
        </Card>
      </div>
      ...
      <div className="lg:col-span-3">
        <Card className="p-4 border border-slate-100 min-h-[300px]">
          {!subject ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-300 py-16">
              <Icon name="grid" size={40} />
              <p className="text-sm mt-3 text-slate-400">اختر مادة لعرض وحداتها ودروسها</p>
            </div>
          ) : (
            <>
                ...
                {units.map((u) => (
                  <div key={u.id} className="border border-slate-100 rounded-xl overflow-hidden">
                    <div className="bg-slate-50 px-4 py-2.5 flex items-center justify-between">
                      <span className="font-bold text-sm text-primary-900">{u.title}</span>
                    ...
                    {lessonsOfUnit(db, u.id).map((l) => (
                      <div key={l.id} className="flex items-center gap-3 px-4 py-2.5 border-t border-slate-50">
                        <Icon name="video" size={15} className="text-slate-300" />
                        ...
                        <button onClick={() => deleteLesson(l.id)} className="p-1 rounded-lg text-slate-300 hover:bg-red-50 hover:text-red-500"><Icon name="trash" size={14} /></button>
                      </div>
                    ))}
                  </div>
                ))}
                {units.length === 0 && <p className="text-xs text-slate-400 text-center py-8">لا وحدات بعد</p>}
              </div>
            </>
          )}
        </Card>
      ...
      <Modal open={addSubOpen} onClose={() => setAddSubOpen(false)} title="إضافة مادة جديدة">
        <div className="space-y-3">
          <div>
            <label className="text-xs font-bold text-slate-500 block mb-1">اسم المادة</label>
          ...
          <div>
            <label className="text-xs font-bold text-slate-500 block mb-1">المعلم</label>
      ...
      <Modal open={!!addLessonOpen} onClose={() => setAddLessonOpen(null)} title="إضافة درس جديد">
        <div className="space-y-3">
          <div>
            <label className="text-xs font-bold text-slate-500 block mb-1">عنوان الدرس</label>
            <input value={lessonTitle} onChange={(e) => setLessonTitle(e.target.value)} placeholder="مثال: قانون نيوتن الأول"
              className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-primary-400" />
          </div>
          <div>
            <label className="text-xs font-bold text-slate-500 block mb-1">المدة (دقائق)</label>
