...
export default function QuestionsTab() {
  ...
  return (
    <div className="grid lg:grid-cols-5 gap-5">
      <Card className="lg:col-span-2 p-4 border border-[#2b3547] h-fit">
        <h3 className="font-extrabold text-white text-sm mb-3">اختر الدرس</h3>
        <select value={subjectId} onChange={(e) => { setSubjectId(e.target.value); setLessonId(""); }}
          className="w-full border border-[#2b3547] rounded-xl px-3 py-2.5 text-sm bg-[#161c29] outline-none focus:border-[#2072e0] mb-3">
        ...
        {!lessonId ? (
          ...
          <>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-extrabold text-white text-sm">أسئلة «{lessons.find(l => l.id === lessonId)?.title}» ({questions.length})</h3>
            ...
            <div className="space-y-2.5 max-h-[420px] overflow-y-auto">
              {questions.map((q, i) => (
                <div key={q.id} className="border border-[#2b3547] rounded-xl p-3.5">
                  <div className="flex items-start gap-3">
                    <span className="w-6 h-6 rounded-lg bg-[#2072e0]/15 text-[#4a9bf5] text-xs flex items-center justify-center font-extrabold shrink-0">{i + 1}</span>
                ...
                {opts.map((o, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <input value={o} onChange={(e) => setOpts(opts.map((x, xi) => (xi === i ? e.target.value : x)))} placeholder={`الخيار ${i + 1}`}
                      className="flex-1 border border-[#2b3547] rounded-xl px-3 py-2 text-sm outline-none focus:border-[#2072e0]" />
                  </div>
                ))}
              </div>
            </div>
